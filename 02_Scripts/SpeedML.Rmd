...
```
```{r input dataset}
df_150 <- read.csv("../01_Data/02_Processed/randomForest/TestData_150.csv")
...
predictions_150 <- predict(forest, newdata = df_150)
```
```{r add the column back to test data}
df_exact$Speed_mph <- predictions_exact
df_150$Speed_mph <- predictions_150
```
...
m <- leaflet(df_exact) %>%
  ...
  addCircleMarkers(lng = ~x_coordinate, lat = ~y_coordinate,
                   fillColor = ~mypalette(Speed_mph), radius=5, stroke=FALSE) %>%
...
```{r}
map <-  addMarkers(map, df_station$x_coordinate, df_station$y_coordinate, popup = df_station$Station_Name)
map
...
```{r}
write.csv(df_exact, "../01_Data/02_Processed/randomForest/Predection_exact.csv", row.names = FALSE)
write.csv(df_150, "../01_Data/02_Processed/randomForest/Predection_150.csv", row.names = FALSE)
```
